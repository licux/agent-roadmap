...
v1.0 では、**ADK の Python 版、Java 版と同じことが Kotlin でも一通りできる**ようになっています。階層的なマルチエージェント構成、Human-in-the-Loop による承認、長い対話のコンテキスト圧縮、中断したセッションの再開などが利用できます。また、ツールの定義も、アノテーションを付けるだけで済むようになりました。
